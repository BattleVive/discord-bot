...
def test_identity_matching_prefers_exact_account_name_over_display_name() -> None:
    """Verify that identity matching prefers exact account name over display name."""
...
async def test_identity_saving_binds_member_number_only_for_a_unique_guild_member() -> None:
    """Verify that identity saving binds member number only for a unique guild member."""
    ...
    class Identities:
        """Provide a identities test double."""
        async def bind(self, member_number: int, discord_id: int, provenance: str) -> bool:
            """Provide bind behavior for the test scenario."""
