from io import BytesIO
from pathlib import Path

# pyrefly: ignore [missing-import]
from PIL import Image, ImageDraw, ImageFont

# ── Asset paths ────────────────────────────────────────────────────────────────
_ASSETS      = Path(__file__).parent / "assets"
FONT_BOLD    = str(_ASSETS / "fonts" / "DejaVuSans-Bold.ttf")
FONT_REGULAR = str(_ASSETS / "fonts" / "DejaVuSans.ttf")

# ── Card dimensions ────────────────────────────────────────────────────────────
CARD_W, CARD_H = 480, 130
PAD            = 16
PFP_SIZE       = 58
PFP_BORDER     = 2
CONTENT_X      = PAD + PFP_SIZE + 14   # 88
BAR_H          = 8
CORNER_R       = 8

# ── Palette ────────────────────────────────────────────────────────────────────
C_BG          = (30,  33,  40)
C_BORDER      = (58,  61,  69)
C_TEXT        = (242, 243, 245)
C_MUTED       = (128, 132, 142)
C_SECONDARY   = (181, 186, 193)
C_ACCENT      = (64,  181, 184)
C_ACCENT2     = (126, 182, 255)
C_BAR_BG      = (46,  51,  64)
C_WIN         = (87,  242, 135)
C_GOLD        = (255, 193,   7)
C_GOLD_TEXT   = (26,  26,    0)
C_BADGE_NEXT  = (37,  42,   58)
C_NEXT_TEXT   = (126, 182, 255)
C_NEXT_BORDER = (58,  85,  128)
C_ARROW       = (92,  96,  112)


def _load_fonts() -> dict:
    return {
        "username": ImageFont.truetype(FONT_BOLD,    15),
        "badge":    ImageFont.truetype(FONT_BOLD,    11),
        "meta":     ImageFont.truetype(FONT_REGULAR, 10),
        "stats":    ImageFont.truetype(FONT_REGULAR, 12),
        "stats_b":  ImageFont.truetype(FONT_BOLD,    12),
        "arrow":    ImageFont.truetype(FONT_REGULAR, 13),
    }


def _circle_crop(img: Image.Image, size: int, border_color: tuple, border_w: int) -> Image.Image:
    """Avatar cropped to circle with a coloured border. Returns RGBA."""
    total  = size + border_w * 2
    result = Image.new("RGBA", (total, total), (0, 0, 0, 0))
    ImageDraw.Draw(result).ellipse([0, 0, total - 1, total - 1], fill=border_color)
    avatar = img.convert("RGBA").resize((size, size), Image.LANCZOS)
    mask   = Image.new("L", (size, size), 0)
    ImageDraw.Draw(mask).ellipse([0, 0, size - 1, size - 1], fill=255)
    avatar.putalpha(mask)
    result.paste(avatar, (border_w, border_w), avatar)
    return result


def _gradient_bar(width: int, height: int, c1: tuple, c2: tuple) -> Image.Image:
    """Single-row horizontal gradient stretched to height."""
    row = Image.new("RGB", (width, 1))
    px  = row.load()
    for x in range(width):
        t = x / max(width - 1, 1)
        px[x, 0] = tuple(int(c1[i] + (c2[i] - c1[i]) * t) for i in range(3))
    return row.resize((width, height), Image.NEAREST)


def _text_w(font, text: str) -> int:
    bb = font.getbbox(text)
    return bb[2] - bb[0]


def _draw_badge(
    draw: ImageDraw.Draw, x: int, y: int,
    text: str, font,
    fill: tuple, text_color: tuple,
    outline: tuple = None,
    pad_x: int = 9, pad_y: int = 3,
) -> int:
    """Pill badge. Returns badge width."""
    bb = font.getbbox(text)
    tw, th = bb[2] - bb[0], bb[3] - bb[1]
    bw = tw + pad_x * 2
    bh = th + pad_y * 2
    draw.rounded_rectangle(
        [x, y, x + bw, y + bh], radius=bh // 2,
        fill=fill, outline=outline, width=1 if outline else 0,
    )
    draw.text((x + pad_x - bb[0], y + pad_y - bb[1]), text, font=font, fill=text_color)
    return bw


def build_card(
    avatar:       Image.Image,
    display_name: str,
    rank_current: str,
    rank_next:    str,
    mmr_current:  int,
    mmr_required: int,
    wins:         int,
    losses:       int,
) -> bytes:
    """
    Render a profile card and return PNG bytes.

    Args:
        avatar:       PIL Image of the player's avatar (any size/mode).
        display_name: Name shown on the card.
        rank_current: Label for current rank, e.g. "Rank 1".
        rank_next:    Label for next rank, e.g. "Rank 2".
        mmr_current:  Player's current MMR.
        mmr_required: MMR needed to reach next rank.
        wins:         Total wins.
        losses:       Total losses.

    Returns:
        PNG bytes – pass to discord.File(BytesIO(png), filename="profile.png").
    """
    fonts   = _load_fonts()
    total   = wins + losses
    wr      = round(wins / total * 100) if total > 0 else 0
    mmr_pct = mmr_current / mmr_required

    card = Image.new("RGBA", (CARD_W, CARD_H), (0, 0, 0, 0))
    draw = ImageDraw.Draw(card)
    draw.rounded_rectangle(
        [0, 0, CARD_W - 1, CARD_H - 1],
        radius=CORNER_R, fill=C_BG, outline=C_BORDER, width=1,
    )

    # PFP – vertically centred
    pfp   = _circle_crop(avatar, PFP_SIZE, C_ACCENT, PFP_BORDER)
    pfp_y = (CARD_H - pfp.height) // 2
    card.paste(pfp, (PAD, pfp_y), pfp)

    cx, cy = CONTENT_X, PAD
    bar_w  = CARD_W - cx - PAD

    # Username
    draw.text((cx, cy), display_name, font=fonts["username"], fill=C_TEXT)
    cy += fonts["username"].getbbox(display_name)[3] + 8

    # Rank badges + arrow
    x_cur = cx
    x_cur += _draw_badge(
        draw, x_cur, cy, f"★ {rank_current}",
        fonts["badge"], fill=C_GOLD, text_color=C_GOLD_TEXT,
    ) + 6
    draw.text((x_cur, cy + 3), "→", font=fonts["arrow"], fill=C_ARROW)
    x_cur += _text_w(fonts["arrow"], "→") + 6
    _draw_badge(
        draw, x_cur, cy, rank_next,
        fonts["badge"], fill=C_BADGE_NEXT, text_color=C_NEXT_TEXT,
        outline=C_NEXT_BORDER,
    )
    cy += fonts["badge"].getbbox("A")[3] + 6 + 9

    # MMR meta
    mmr_l = f"{mmr_current:,} MMR"
    mmr_m = f"{round(mmr_pct * 100)}%"
    mmr_r = f"{mmr_required:,} MMR"
    draw.text((cx, cy), mmr_l, font=fonts["meta"], fill=C_MUTED)
    draw.text(
        (cx + bar_w // 2 - _text_w(fonts["meta"], mmr_m) // 2, cy),
        mmr_m, font=fonts["meta"], fill=C_ACCENT,
    )
    draw.text(
        (cx + bar_w - _text_w(fonts["meta"], mmr_r), cy),
        mmr_r, font=fonts["meta"], fill=C_MUTED,
    )
    cy += 13

    # Progress bar
    draw.rounded_rectangle(
        [cx, cy, cx + bar_w, cy + BAR_H],
        radius=BAR_H // 2, fill=C_BAR_BG,
    )
    fill_w    = max(1, int(bar_w * mmr_pct))
    grad      = _gradient_bar(fill_w, BAR_H, C_ACCENT, C_ACCENT2)
    fill_mask = Image.new("L", (fill_w, BAR_H), 0)
    ImageDraw.Draw(fill_mask).rounded_rectangle(
        [0, 0, fill_w + BAR_H, BAR_H - 1],
        radius=BAR_H // 2, fill=255,
    )
    card.paste(grad, (cx, cy), fill_mask)
    cy += BAR_H + 9

    # Stats
    wr_text = f"↑ {wr}% WR"
    wl_text = f"W {wins} · L {losses}"
    draw.text((cx, cy), wr_text, font=fonts["stats_b"], fill=C_WIN)
    draw.text(
        (cx + _text_w(fonts["stats_b"], wr_text) + 14, cy),
        wl_text, font=fonts["stats"], fill=C_SECONDARY,
    )

    out = BytesIO()
    card.convert("RGB").save(out, format="PNG")
    return out.getvalue()
